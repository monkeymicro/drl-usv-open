...
class VectorizedCritic(nn.Module):
    def __init__(
        self, state_dim: int, action_dim: int, hidden_dim: int, num_critics: int, layernorm: bool = False
    ):
        super().__init__()
    ...
    def forward(self, state: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
        ...
        # sa: [batch_size, state_dim + action_dim]
        # output: [ensemble_size, batch_size, 1]
...
class LB_SAC:
    ...
    def train(self, batch: TensorBatch) -> Dict[str, float]:
        ...
        with torch.no_grad():
            ...
            target_q, _ = torch.sort(target_q, dim=0)
            target_q = target_q[: self.num_critics - 1]
            target_q = target_q.mean(0)
            # take min over all critics
            # target_q = target_q.min(0)[0]
            target_q = target_q - self.alpha * next_log_prob.unsqueeze(0)
            target_q = reward + (1.0 - done) * self.gamma * target_q
            target_q = target_q.squeeze(0)

        q_ensemble = self.critics(state, action)
        q_loss = (q_ensemble - target_q.unsqueeze(0)).pow(2).sum(0).mean()
        ...
        # take min over the first num_critics - 1 critics
        q_ensemble, _ = torch.sort(q_ensemble, dim=0)
        q_ensemble = q_ensemble[: self.num_critics - 1]
        q_ensemble = q_ensemble.mean(0)
        # take min over all critics
        # q_ensemble = q_ensemble.min(0)[0]

        actor_loss = (self.alpha * log_prob - q_ensemble).mean()
...
def train(config: TrainConfig):
    ...
    critics = VectorizedCritic(state_dim, action_dim, config.hidden_dim, config.num_critics, config.critic_layernorm).to(config.device)
    ...
    print("---------------------------------------")
    print(f"Training LB-SAC")
    print("---------------------------------------")

    # Initialize policy
    trainer = LB_SAC(**kwargs)
